...
export async function GET(request: Request) {
    try {
        ...
        const yearRes = await db.query('SELECT id, year_name FROM academic_years WHERE is_active = true AND tenant_id = $1 LIMIT 1', [auth.tenant_id]);
        ...
        const studentRes = await db.query(
            ...
            [activeYear.id, studentId, auth.tenant_id]
        );
