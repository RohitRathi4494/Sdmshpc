...
export async function getStudentReportData(student_id: number, academic_year_id: number, tenant_id: number) {
  // 1. Student Info
  const studentQuery = `
    ...
    WHERE s.id = $1 AND se.academic_year_id = $2 AND s.tenant_id = $3
  `;
  const studentRes = await db.query(studentQuery, [student_id, academic_year_id, tenant_id]);
  ...
  const scholasticQuery = `
    ...
    WHERE ss.student_id = $1 AND ss.academic_year_id = $2 AND ss.tenant_id = $3
  `;
  const scholasticRes = await db.query(scholasticQuery, [student_id, academic_year_id, tenant_id]);
  ...
  const coScholasticQuery = `
    ...
    WHERE css.student_id = $1 AND css.academic_year_id = $2 AND css.tenant_id = $3
  `;
  const coScholasticRes = await db.query(coScholasticQuery, [student_id, academic_year_id, tenant_id]);
  ...
  const attendanceQuery = `
    ...
    WHERE ar.student_id = $1 AND ar.academic_year_id = $2 AND ar.tenant_id = $3
    ORDER BY m.display_order ASC
  `;
  const attendanceRes = await db.query(attendanceQuery, [student_id, academic_year_id, tenant_id]);
  ...
  const remarksQuery = `
    ...
    WHERE r.student_id = $1 AND r.academic_year_id = $2 AND r.tenant_id = $3
  `;
  const remarksRes = await db.query(remarksQuery, [student_id, academic_year_id, tenant_id]);
  ...
  if (isXiOrXii) {
    const studentSubjectsQuery = `
          ...
          WHERE ss.student_id = $3 AND ss.academic_year_id = $2 AND ss.tenant_id = $4
          ORDER BY subject_sort_order ASC, cs.display_order ASC, sub.subject_name ASC
      `;
    subjectsRes = await db.query(studentSubjectsQuery, [student.class_id, academic_year_id, student_id, tenant_id]);
    ...
    if (subjectsRes.rows.length === 0) {
      const subjectsQuery = `
            ...
            WHERE cs.class_id = $1 AND cs.academic_year_id = $2 AND cs.tenant_id = $3
            ORDER BY cs.display_order ASC, sub.subject_name ASC
        `;
      subjectsRes = await db.query(subjectsQuery, [student.class_id, academic_year_id, tenant_id]);
    }
  } else {
    const subjectsQuery = `
        SELECT sub.id, sub.subject_name, cs.max_marks, cs.assessment_max_marks
